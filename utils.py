...
def make_train_test_mol_orphan(df,nb_folds):
  ...
  for train_index, test_index in group_kfold.split(range(nb_positive_inter), groups=groups):
      ...
      I = [i for i,elt in enumerate(test) for x in train if elt[1]==x[1]]
...
def make_train_test_prot_orphan(df,nb_folds):
  ...
  for train_index, test_index in group_kfold.split(range(nb_positive_inter), groups=groups):
      ...
      # we remove the interactions with proteins in test that are in train
      I = [i for i,elt in enumerate(test) for x in train if elt[0]==x[0]]
      test =np.delete(test, list(set(I)) ,axis = 0)
...
def make_train_test_prot_mol_orphan(df,nb_folds):
    """
    make train and test sets
    ...
    print("Train/test datasets prepared.")
    with open('data/train_arr_prot_mol_orphan.data', 'wb') as f:
        pickle.dump(all_train_interactions_arr, f)
    with open('data/test_arr_prot_mol_orphan.data', 'wb') as f:
        pickle.dump(all_test_interactions_arr, f)
    return all_train_interactions_arr, all_test_interactions_arr
